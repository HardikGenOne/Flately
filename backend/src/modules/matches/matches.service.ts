...
export class MatchesService {
  constructor(
    private readonly dependencies: MatchesServiceDependencies = {
      findMatchesForUser,
    },
  ) {}

  async checkAndCreateMatch(fromUserId: string, toUserId: string): Promise<MatchCheckResult> {
  ...
  async getMyMatches(userId: string) {
    ...
    const compatibilityPairs = await this.dependencies.findMatchesForUser(userId);
...
export async function checkAndCreateMatch(
  ...
  return matchesService.checkAndCreateMatch(fromUserId, toUserId);
}

export async function getMyMatches(userId: string) {
  return matchesService.getMyMatches(userId);
}
