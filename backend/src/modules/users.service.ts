...
export class UsersService {
  async getOrCreateUser(data: AuthProfileInput) {
    const normalizedEmail = this.normalizeEmail(data.email);

    let user = this.isObjectId(data.userId)
  ...
  private isObjectId(value: string | undefined): value is string {
...
export async function getOrCreateUser(data: AuthProfileInput) {
  return usersService.getOrCreateUser(data);
}
